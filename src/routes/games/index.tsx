...
import {
  ...
  Select, Slider, Stack, TextInput, Anchor, Pagination,
  AspectRatio, Skeleton, UnstyledButton,
} from '@mantine/core'
...
function RouteComponent() {
  ...
  const currentStatus: Status = !search.dates
    ? 'all'
    : search.dates.startsWith('2000')
    ? 'released'
    : 'upcoming'
  ...
  const setStatus = (status: Status) => {
    ...
    const dates =
      status === 'released' ? `2000-01-01,${today}` :
      status === 'upcoming' ? `${tomorrow},2030-12-31` :
      undefined
  ...
  const togglePlatform = (id: string) => {
    const next = currentPlatforms.includes(id)
      ? currentPlatforms.filter((p) => p !== id)
      : [...currentPlatforms, id]
                ...
                <Box style={{ display: 'flex', flexWrap: 'wrap', gap: '7px 6px' }}>
                  {GENRES.map((g) => (
                    <FilterChip
                      key={g.slug}
                      label={g.label}
                      active={currentGenres.includes(g.slug)}
                ...
                <Box style={{ display: 'flex', flexWrap: 'wrap', gap: '7px 6px' }}>
                  {PLATFORMS.map((p) => (
                    <FilterChip
                      key={p.id}
                      label={p.label}
                      active={currentPlatforms.includes(p.id)}
        ...
        <Box style={{ flex: 1, minWidth: 0 }}>
          <Group justify="space-between" mb="md" align="center">
            <Group gap="sm" align="center">
              ...
              <UnstyledButton
                ...
                style={{
                  ...
                  background: filtersOpen
                    ? 'color-mix(in oklab, #8B6BFF 15%, #131A38)'
              ...
              {hasFilters && (
                <Anchor component="button" fz="xs" c="violet.4" fw={500} onClick={clearAll}
